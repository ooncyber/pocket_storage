        ...
        <v-row>
          <v-col>
            <div class="embed-responsive embed-responsive-16by9">
              <video
                controls
                :src="'https://tesca.loca.lt/public/' + video.path"
              ></video>
            </div>
          </v-col>
        </v-row>
...
export default {
  ...
  async mounted() {
    let videos = await (await fetch("https://tesca.loca.lt/videos")).json();
    this.listaCategorias = await (
      await fetch("https://tesca.loca.lt/categorias")
    ).json();
    this.videos = videos;
  ...
  methods: {
    async buscar(i) {
      this.videos = await (
        await fetch("https://tesca.loca.lt/categorias/" + i.categoria)
      ).json();
